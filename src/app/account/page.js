...
function EditProfileForm({ user, onCancel, onSave }) {
  ...
  const [form, setForm] = useState({
    ...
    email: user.email || "",
    phone: user.phone || "",
    billingAddress: typeof user.billingAddress === 'string' ? user.billingAddress : (user.billingAddress?.address || ""),
    shippingAddress: typeof user.shippingAddress === 'string' ? user.shippingAddress : (user.shippingAddress?.address || ""),
  ...
  function validate(fields) {
    ...
    else if (!/^[\d\-+() ]{7,20}$/.test(fields.phone)) errors.phone = "Invalid phone format.";
    if (!fields.billingAddress.trim()) errors.billingAddress = "Billing address is required.";
    if (!fields.shippingAddress.trim()) errors.shippingAddress = "Shipping address is required.";
    if (showPasswordFields) {
      if (!fields.currentPassword) errors.currentPassword = "Current password required.";
      if (!fields.newPassword) errors.newPassword = "New password required.";
      else if (fields.newPassword.length < 8) errors.newPassword = "Password must be at least 8 characters.";
      else if (!/[0-9]/.test(fields.newPassword)) errors.newPassword = "Password must contain at least one digit.";
  ...
  return (
    ...
    >
      <div className="flex gap-8">
        <div className="flex-1 flex flex-col gap-3">
          <label className="text-sm font-medium text-[#888]">
            First Name
            <input className={`mt-1 w-full rounded border px-2 py-1 ${fieldErrors.firstName ? 'border-red-400' : ''}`} name="firstName" value={form.firstName} onChange={handleChange} required />
            {fieldErrors.firstName && <span className="text-xs text-red-500">{fieldErrors.firstName}</span>}
          </label>
          <label className="text-sm font-medium text-[#888]">
            Last Name
            <input className={`mt-1 w-full rounded border px-2 py-1 ${fieldErrors.lastName ? 'border-red-400' : ''}`} name="lastName" value={form.lastName} onChange={handleChange} required />
            {fieldErrors.lastName && <span className="text-xs text-red-500">{fieldErrors.lastName}</span>}
          </label>
          <label className="text-sm font-medium text-[#888]">
            E-mail
            <input className={`mt-1 w-full rounded border px-2 py-1 ${fieldErrors.email ? 'border-red-400' : ''}`} name="email" type="email" value={form.email} onChange={handleChange} required />
            {fieldErrors.email && <span className="text-xs text-red-500">{fieldErrors.email}</span>}
          </label>
          <label className="text-sm font-medium text-[#888]">
            Telephone number
            <input className={`mt-1 w-full rounded border px-2 py-1 ${fieldErrors.phone ? 'border-red-400' : ''}`} name="phone" value={form.phone} onChange={handleChange} required />
        ...
        <div className="flex-1 flex flex-col gap-3 border-l pl-8">
          <label className="text-sm font-medium text-[#888]">
            Billing Address
            <input className={`mt-1 w-full rounded border px-2 py-1 ${fieldErrors.billingAddress ? 'border-red-400' : ''}`} name="billingAddress" value={form.billingAddress} onChange={handleChange} required />
            {fieldErrors.billingAddress && <span className="text-xs text-red-500">{fieldErrors.billingAddress}</span>}
          </label>
          <label className="text-sm font-medium text-[#888]">
            Shipping Address
            <input className={`mt-1 w-full rounded border px-2 py-1 ${fieldErrors.shippingAddress ? 'border-red-400' : ''}`} name="shippingAddress" value={form.shippingAddress} onChange={handleChange} required />
            {fieldErrors.shippingAddress && <span className="text-xs text-red-500">{fieldErrors.shippingAddress}</span>}
          </label>
          <div className="mt-6">
            <button
              type="button"
      ...
      <div className="flex gap-4 mt-4">
        <button type="button" className="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold" onClick={onCancel} disabled={loading}>Cancel</button>
        <button type="submit" className="px-4 py-2 rounded bg-[#111] hover:bg-[#222] text-white font-semibold" disabled={loading}>{loading ? "Saving..." : "Save"}</button>
      </div>
    </motion.form>
  );
}
    ...
    <label className="text-sm font-medium text-[#888] w-full">
      ...
      <div className="relative mt-1 w-full">
        <input
          ...
          className={`w-full rounded border px-2 py-1 pr-10 ${error ? 'border-red-400' : ''}`}
          name={name}
          value={value}
          onChange={onChange}
          autoComplete={name}
        ...
        >
          {visible ? (
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 002.25 12c0 .621.113 1.223.323 1.777M6.32 6.319A10.45 10.45 0 0112 5.25c6.75 0 9.75 6.75 9.75 6.75a10.48 10.48 0 01-4.293 5.226M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-gray-500 hover:text-[#222]">
            <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487a2.1 2.1 0 1 1 2.97 2.97L7.5 19.79l-4 1 1-4 14.362-14.303ZM19.5 7.5l-2-2" />
          </svg>
        </button>
      </div>
  ...
  return (
    <div className="bg-white min-h-[calc(100vh-10rem)] mt-10 py-10 flex justify-center">
      <div className="flex w-full max-w-6xl gap-8">
        {/* Sidebar */}
        <aside className="w-64 bg-[#f5f5f7] rounded-md shadow-md py-6 px-2 flex flex-col gap-2 justify-between" style={{background: 'rgba(245,245,247,0.95)', minHeight: '500px'}}>
          <div>
            <span className="text-xl font-semibold tracking-widest mb-6 ml-1 text-[#222]">ACCOUNT</span>
            {TABS.map((tab) => (
              <button
                key={tab.key}
                className={`block w-full text-left px-2 py-2 my-1 rounded-xl text-[15px] font-medium tracking-wide transition-all duration-300 ${activeTab === tab.key ? "bg-white/30 font-bold text-[#222] shadow-lg ring-2 ring-[#e0e0e0] backdrop-blur-xl" : "text-[#888] hover:text-[#222]"}`}
          ...
          <motion.button
            whileHover={{ scale: 1.09, boxShadow: '0 4px 24px 0 rgba(0,0,0,0.10)' }}
            ...
            style={{ boxShadow: '0 8px 32px 0 rgba(31,38,135,0.10)', background: 'rgba(255,255,255,0.25)', border: '1px solid rgba(255,255,255,0.18)', backdropFilter: 'blur(12px)', borderRadius: '18px' }}
        ...
        <section className="flex-1 bg-[#f5f5f7] rounded-md shadow-md p-8 min-h-[320px] flex items-center justify-center transition-all duration-500" style={{ minHeight: 320, height: 'auto', background: 'rgba(245,245,247,0.95)' }}>
          <AnimatePresence mode="wait">
            {activeTab === "profile" && user && (
              ...
              >
                {editing ? (
                  <EditProfileForm user={user} onCancel={() => setEditing(false)} onSave={handleProfileSave} />
                ) : (
